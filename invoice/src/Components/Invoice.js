...
export default function Invoice() {
    ...
    //calcutating total on change
    const upadte = () => {
        var arr = document.getElementsByName("amount");
        var subtotal = 0;
        for (var i = 0; i < invoiceData.items.length; i++) {
            console.log(parseInt(invoiceData.items[i]))
            subtotal += parseFloat((invoiceData.items[i].quantity * invoiceData.items[i].unitPrice) - (invoiceData.items[i].quantity * invoiceData.items[i].unitPrice) * invoiceData.items[i].discount / 100);
    ...
    const sendMail = (mail) => {
        ...
        axios.post(`http://localhost:8899/api/mail/${remail}`, data, { headers: { 'content-type': 'multipart/form-data' } })
    }
    //send data to api
    const handleadder = () => {
    ...
    return (
        <>
                ...
                <div ref={pdfRef}>
                    <Paper elevation={3} sx={{ width: "70%", mx: "auto", textAlign: 'center', '&>:not(styled)': { fontFamily: "'Barlow Condensed', sans-serif" } }}>
                        ...
                        <Grid container spacing={2}>
                            ...
                            <Grid item xs={6} md={6} >
                                {filled ? <Box sx={{ textAlign: 'left', paddingLeft: "20px" }}>
                                    <Typography variant='h6'>Bill to :
                                ...
                                >
                                    ...
                                    <DialogContent>
                                        <Stack spacing={2} sx={{ width: "500px", paddingY: "10px" }}>
                                            ...
                                            <TextField label="Email" onChange={(e) => { setRemail(e.target.value) }} variant="outlined" />
            ...
            <ReactToPdf targetRef={pdfRef} filename={`${sname}_invoice.pdf`} options={options} x={-1} y={0} scale={0.8}>
                {({ toPdf }) => (
                    <Button mx={2} onClick={() => { toPdf(); handleadder(); }} variant="contained" endIcon={<SaveIcon />}>
                        raise Invoice
                    </Button>
                )}
            </ReactToPdf>
